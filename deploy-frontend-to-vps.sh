...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  1. Полностью закройте Telegram на телефоне (свайп вверх)"
        echo "  2. Подождите 5 секунд"
        echo "  3. Откройте Telegram снова"
        echo "  4. Откройте бота и WebApp"
        echo ""
        info "⚠️  Если изменения всё ещё не видны:"
        echo "  1. Очистите кэш Telegram в настройках"
        echo "  2. Или добавьте ?v=2 к URL WebApp в @BotFather"
        echo ""
